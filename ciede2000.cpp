...
template<typename T>
T ciede2000(T l1, T a1, T b1, T l2, T a2, T b2, T kl = T(1.0), T kc = T(1.0), T kh = T(1.0), bool canonical = false) {
	...
	h1 += (h1 < T(0.0)) * T(2.0) * pi;
	h2 += (h2 < T(0.0)) * T(2.0) * pi;
	...
	if (pi + epsilon < std::fabs(h2 - h1)) {
		h_delta += pi;
		if (canonical && pi + epsilon < h_mean)
	...
	const T p = T(36.0) * h_mean - T(55.0) * pi;
	n = (c1 + c2) * T(0.5);
	n = n * n * n * n * n * n * n;
	// The hue rotation correction term is designed to account for the
	// non-linear behavior of hue differences in the blue region.
	const T r_t = T(-2.0) * std::sqrt(n / (n + T(6103515625.0)))
		* std::sin(pi / T(3.0) * std::exp(p * p / (T(-25.0) * pi * pi)));
	n = (l1 + l2) * T(0.5);
	n = (n - T(50.0)) * (n - T(50.0));
	// Lightness.
	const T l = (l2 - l1) / (kl * (T(1.0) + T(3.0)
		/ T(200.0) * n / std::sqrt(T(20.0) + n)));
	// These coefficients adjust the impact of different harmonic
	// components on the hue difference calculation.
	const T t = T(1.0)
		- T(17.0) / T(100.0) * std::sin(h_mean + pi / T(3.0))
		+ T(6.0) / T(25.0) * std::sin(T(2.0) * h_mean + pi / T(2.0))
		+ T(8.0) / T(25.0) * std::sin(T(3.0) * h_mean + T(8.0) * pi / T(15.0))
		- T(1.0) / T(5.0) * std::sin(T(4.0) * h_mean + T(3.0) * pi / T(20.0));
	n = c1 + c2;
	// Hue.
	const T h = T(2.0) * std::sqrt(c1 * c2) * std::sin(h_delta)
		/ (kh * (T(1.0) + T(3.0) / T(400.0) * n * t));
	// Chroma.
